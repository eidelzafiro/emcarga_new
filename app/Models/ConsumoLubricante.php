<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;

class ConsumoLubricante extends Model
{
    protected $table = 'consumo_lubricantes';

    protected $fillable = [
        'folio', 'id_tractivo', 'id_tipo_aceite', 'id_causa',
        'cantidad', 'unidad', 'importe_mn', 'importe_me', 'fecha',
    ];

    protected function casts(): array
    {
        return [
            'fecha' => 'date',
            'cantidad' => 'decimal:2',
            'importe_mn' => 'decimal:2',
            'importe_me' => 'decimal:2',
        ];
    }

    public function tractivo(): BelongsTo
    {
        return $this->belongsTo(Tractivo::class, 'id_tractivo');
    }

    public function tipoAceite(): BelongsTo
    {
        return $this->belongsTo(TipoLubricante::class, 'id_tipo_aceite');
    }

    public function causa(): BelongsTo
    {
        return $this->belongsTo(TipoCausa::class, 'id_causa');
    }
}
