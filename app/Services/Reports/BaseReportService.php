...
abstract class BaseReportService
{
    ...
    protected function streamPdf(string $view, array $data = [], ?string $filename = null): \Illuminate\Http\Response
    {
        $filename ??= str_replace(' ', '_', $this->title) . '.pdf';
        return $this->pdf($view, $data)->stream($filename);
    }

    protected function downloadPdf(string $view, array $data = [], ?string $filename = null): \Illuminate\Http\Response
    {
        $filename ??= str_replace(' ', '_', $this->title) . '.pdf';
        return $this->pdf($view, $data)->download($filename);
    }

    protected function downloadExcel(string $exportClass, ?string $filename = null): \Illuminate\Http\Response
    {
