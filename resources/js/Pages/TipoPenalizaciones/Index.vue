...
const props = defineProps({ tipoPenalizaciones: Object, filters: Object })
...
watch(search, () => {
    router.get(route('tipo-penalizaciones.index'), { search: search.value }, { preserveState: true, replace: true })
})
...
function submit() {
    const url = editing.value ? route('tipo-penalizaciones.update', editing.value.id) : route('tipo-penalizaciones.store')
...
<template>
    <AppLayout :title="title">
        <div class="card">
            ...
            <DataTable :value="tipoPenalizaciones.data" striped-rows paginator :rows="20" :total-records="tipoPenalizaciones.total">
                ...
                <Column header="Acciones" style="width: 120px">
                    <template #body="{ data }">
                        <div class="flex gap-1">
                            ...
                            <Button icon="pi pi-trash" rounded text severity="danger" @click="router.delete(route('tipo-penalizaciones.destroy', data.id))" />
                        </div>
                    </template>
                </Column>
            </DataTable>
        ...
        <Dialog v-model:visible="showForm" :header="editing ? 'Editar Tipo de Penalización' : 'Nuevo Tipo de Penalización'" modal style="width: 500px">
